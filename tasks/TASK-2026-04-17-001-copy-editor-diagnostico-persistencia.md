...
⚠️ 2 campos menores ausentes.

---

### ETAPA6 — Identidade visual

| Campo | Tipo UI | Editável | Obs |
|-------|---------|:-:|-----|
| header.* | STRING | ✓ | |
| intro.* | STRING/TEXTAREA | ✓ | |
| diferenca | NESTED_OBJECT | ✓ | |
| itensTitle | STRING | ✓ | |
| items | OBJECT_ARRAY | ✓ | precisa add/remove |
| reassuringTip | TEXTAREA | ✓ | |
| acknowledgement | TEXTAREA | ✓ | |
| navConfirm | STRING | ✓ | |
| completionTitle | STRING | ✓ | |
| completionDescription | TEMPLATE | ⚠️ | é função no copy.js; deep-merge preserva a função — edição não persiste |

⚠️ `completionDescription` — editor mostra como TEMPLATE mas é função; nunca será salva no banco.
...
|-------|---------|:-:|-----|
| header.* | STRING | ✓ | |
| bonificacaoTitle/Body/startKitInfo | TEXTAREA | ✓ | |
| comoFunciona.* | STRING/TEXTAREA | ✓ | |
| logo* | STRING | ✓ | |
| nav* | STRING | ✓ | |
| processingMessages | STRING_ARRAY | ✓ | |
| completionDone/Pending.* | STRING/TEXTAREA | ✓ | |
| modoSimplificado.* | STRING | ✗ | labels/errors de site e instagram ausentes |
| colorPalette* | STRING | ✗ | 5 campos de UI ausentes |

⚠️ ~7 campos ausentes (labels de UI secundários).

---

### ETAPA_FINAL — Parabéns

| Campo | Tipo UI | Editável | Obs |
|-------|---------|:-:|-----|
| resumo.* (5 campos) | STRING | ✓ | |
| proximosPassosLabel | STRING | ✓ | |
| ctaButton/ctaMicro | STRING/TEXTAREA | ✓ | |
| parabens.badge/title/body/cta/closing | STRING/TEXTAREA | ✓ | |
| atendenteLabel | STRING | ⚠️ | **é função no copy.js** — editor exibe undefined |
| nextSteps | FUNÇÃO | ✗ | `(atendente, genero) => [...]` |
| atendenteContactTime | STRING | ✗ | não mapeado |
| parabens.stepLabel | STRING | ✗ | não mapeado |
| parabens.atendenteContact | STRING | ✗ | não mapeado |

⚠️ `atendenteLabel` mapeada incorretamente — causa undefined no editor.

---

## Ordem de execução recomendada

1. **Fix Bug #1** — `update-onboarding-copy/index.ts`: merge content antes do UPDATE (1 arquivo, 5 linhas)
2. **Fix Bug #3** — `EtapaSection.jsx`: remover ou corrigir `atendenteLabel` no schema de etapaFinal
3. **Fix Bug #2** — `EtapaSection.jsx`: adicionar campos de slide3/slide4 da ETAPA4
4. **Fix Bug #4** — `EtapaSection.jsx`: adicionar campos menores por etapa
5. **Fix Bug #5** — melhorias UX (add/remove em listas + input sizing)
...
| Arquivo | Bugs |
|---------|------|
| `supabase/functions/update-onboarding-copy/index.ts` | #1 |
| `src/pages/CopyEditor/EtapaSection.jsx` | #2, #3, #4 |
| `src/pages/CopyEditor/FieldEditor.jsx` (ou EditableObjectList) | #5 |
| `src/copy.js` | contexto/referência |

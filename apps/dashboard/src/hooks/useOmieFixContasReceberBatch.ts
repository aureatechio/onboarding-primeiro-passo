...
export interface OmieFixBatchItem {
  id: string
  compra_id: string
  ...
  result_case_type: string | null
}

export interface OmieFixBatchSummary {
...
export interface OmieFixBatchRun {
  id: string
...
interface UseOmieFixContasReceberBatchReturn {
  loading: boolean
  running: boolean
  error: string | null
  run: OmieFixBatchRun | null
  summary: OmieFixBatchSummary | null
  items: OmieFixBatchItem[]
...
function mapBatchError(code?: string, message?: string): string {
  switch (code) {
    ...
    default:
      return message ?? 'Falha ao executar lote de correção OMIE.'
  }
}
...
export function useOmieFixContasReceberBatch(): UseOmieFixContasReceberBatchReturn {
  const [loading, setLoading] = useState(false)
  const [running, setRunning] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const [run, setRun] = useState<OmieFixBatchRun | null>(null)
  const [summary, setSummary] = useState<OmieFixBatchSummary | null>(null)
  const [items, setItems] = useState<OmieFixBatchItem[]>([])
  ...
  const callBatch = useCallback(
    async (
      ...
      const response = await fetch(
        `${SUPABASE_URL}/functions/v1/omie-fix-contas-receber-batch`,
        {
          method: 'POST',
          headers: {
  ...
  const runProcessLoop = useCallback(
    async (
      ...
      while (true) {
        ...
        const runPayload = (statusRes.run ?? null) as OmieFixBatchRun | null
        const summaryPayload =
          (statusRes.summary ?? null) as OmieFixBatchSummary | null
        const itemsPayload = (statusRes.items ?? []) as OmieFixBatchItem[]

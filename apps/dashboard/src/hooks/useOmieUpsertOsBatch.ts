...
type BatchItemStatus = 'pending' | 'running' | 'success' | 'failed' | 'skipped'
type BatchRunStatus = 'pending' | 'running' | 'completed' | 'failed'

export interface OmieBatchItem {
  id: string
  compra_id: string
...
export interface OmieBatchSummary {
...
interface UseOmieUpsertOsBatchReturn {
  loading: boolean
  running: boolean
  error: string | null
  run: OmieBatchRun | null
  summary: OmieBatchSummary | null
  items: OmieBatchItem[]
...
function mapBatchError(code?: string, message?: string): string {
  switch (code) {
    ...
    default:
      return message ?? 'Falha ao executar lote de Upsert OS.'
  }
}
...
function applyTransactionsFiltersToIdsQuery(filters: TransactionsFilters) {
  ...
  if (filters.celebridade && filters.celebridade !== 'all') {
    if (filters.celebridade === '__none__') {
      ...
      next = next.filter('clicksign_metadata->>celebridade', 'eq', filters.celebridade)
    }
  }
  ...
  if (filters.nfeStatus && filters.nfeStatus !== 'all') {
    switch (filters.nfeStatus) {
      ...
      case 'awaiting':
        ...
        break
      case 'Error':
        next = next.or('nfe_status.eq.Error,nfe_status.eq.Cancelled,nfe_request_status.eq.failed')
        break
      case 'none':
        next = next.is('nfe_status', null).or('nfe_request_status.is.null,nfe_request_status.eq.pending')
        break
    }
  }
...
async function resolveCompraIdsForAllFiltered(
  ...
  // Supabase range is inclusive: 0..99 => 100 rows.
...
export function useOmieUpsertOsBatch(): UseOmieUpsertOsBatchReturn {
  const [loading, setLoading] = useState(false)
  const [running, setRunning] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const [run, setRun] = useState<OmieBatchRun | null>(null)
  const [summary, setSummary] = useState<OmieBatchSummary | null>(null)
  const [items, setItems] = useState<OmieBatchItem[]>([])
  const [executionStartedAtMs, setExecutionStartedAtMs] = useState<number | null>(null)
  ...
  const callBatch = useCallback(
    async (adminPassword: string, payload: Record<string, unknown>): Promise<Record<string, unknown>> => {
      ...
      const response = await fetch(`${SUPABASE_URL}/functions/v1/omie-upsert-os-batch`, {
        method: 'POST',
        headers: {
  ...
  const runProcessLoop = useCallback(
    async (
      ...
      while (true) {
        ...
        const runPayload = (statusRes.run ?? null) as OmieBatchRun | null
        const summaryPayload = (statusRes.summary ?? null) as OmieBatchSummary | null
        const itemsPayload = (statusRes.items ?? []) as OmieBatchItem[]
    ...
    async (params: StartBatchParams): Promise<boolean> => {
      ...
      try {
        ...
        if (params.mode === 'all_filtered') {
          ...
          const { ids, total } = await resolveCompraIdsForAllFiltered(params.filters)
        ...
        setError(err instanceof Error ? err.message : 'Falha ao iniciar lote OMIE.')
        setRunning(false)
        setLoading(false)
      setExecutionStartedAtMs(null)
        return false
  ...
  const retryFailed = useCallback(
    async (adminPassword: string): Promise<boolean> => {
      ...
      try {
        ...
        setItems([])
        setSummary(null)
        await runProcessLoop(retryRunId, adminPassword.trim(), retryRequestedCount)
        setLoading(false)
        return true
      } catch (err) {
  ...
  const progressPercent = useMemo(() => {
    ...
    return Math.min(100, Math.max(0, (summary.processed_count / requestedCount) * 100))

...
import {
  useOmieFixContasReceberBatch,
  type FixExecutionMode,
} from '@/hooks/useOmieFixContasReceberBatch'

type SelectionMode = 'page' | 'all_filtered'

interface OmieFixContasReceberBatchDialogProps {
  open: boolean
...
export function OmieFixContasReceberBatchDialog({
  open,
  onOpenChange,
  mode,
  selectedIds,
  ...
  filters,
  onFinished,
}: OmieFixContasReceberBatchDialogProps) {
  ...
  const batch = useOmieFixContasReceberBatch()
      ...
      <DialogContent className="sm:max-w-[760px]">
        ...
        <div className="space-y-4 py-2">
          ...
          <div className="space-y-2">
            <label
              htmlFor="fix-cr-batch-admin-password"
              className="text-sm font-medium"
            >
              Senha de admin
            </label>
            <PasswordInput
              id="fix-cr-batch-admin-password"
                ...
                <tbody>
                  {batch.items.map((item) => (
                    <tr key={item.id} className="border-t">
                      ...
                      <td className="px-2 py-1">{item.result_case_type ?? '—'}</td>
                      <td className="px-2 py-1">{item.result_action ?? '—'}</td>
                      <td className="px-2 py-1">
                        {item.error_code
                          ? `${item.error_code}${

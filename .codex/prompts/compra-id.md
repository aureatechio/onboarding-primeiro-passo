Busque a compra informada pelo usuario (UUID) via ferramentas/app Supabase disponiveis no Codex com **maxima performance** e **somente leitura**.
...
2) Minimizar roundtrips de ferramenta (ideal: **1 query principal** + no maximo **2** queries extras).
...
- **Erro de upload/API (Edge):** usar ferramenta de logs Supabase, quando disponivel, para funcoes como `save-onboarding-identity`, `save-campaign-briefing`, `get-onboarding-data` quando o problema for falha de funcao, nao linha ausente no SQL.

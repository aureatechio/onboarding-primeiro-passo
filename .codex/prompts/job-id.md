Busque o job de campanha IA informado pelo usuario (UUID de `ai_campaign_jobs.id`) via ferramentas/app Supabase disponiveis no Codex com **maxima performance** e **somente leitura**.
...
2) Minimizar roundtrips de ferramenta (ideal: **2 queries** + no maximo **1** query extra).
...
- **Query 2 — Assets + Erros do job (duas subconsultas em uma):**
  Fazer as duas queries em paralelo (ou sequencial se a ferramenta disponivel nao suportar):
...
- **Logs Edge:** usar apenas se houver erro suspeito e o usuario pedir, para funcoes como `create-ai-campaign-job`, `generate-ai-campaign-image`, `retry-ai-campaign-assets`.

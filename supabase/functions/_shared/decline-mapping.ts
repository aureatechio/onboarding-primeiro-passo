...
function matchRule(rules: DeclineRule[], code: string, message?: string | null): DeclineRule | null {
  const normalizedMessage = normalizeText(message);
  const exact = rules.filter((rule) => rule.code === code);
  const withMatch = exact.find((rule) => rule.messageMatch && normalizedMessage.includes(rule.messageMatch));
...
export function classifyDecline(input: {
  ...
  if (brandKey && providerCode) {
    const rule = matchRule(BRAND_RULES[brandKey], providerCode, providerMessage || returnMessage || reasonMessage);
    if (rule) {
      ...
      return {
        ...
        matchedCode: providerCode,
      };
    }
  }

  if (brandKey && returnCode) {
    const rule = matchRule(BRAND_RULES[brandKey], returnCode, returnMessage || providerMessage || reasonMessage);
    if (rule) {
      const meta = CATEGORY_META[rule.category];
      return {
        category: rule.category,
  ...
  if (returnCode) {
    const rule = matchRule(RETURN_CODE_RULES, returnCode, returnMessage || providerMessage || reasonMessage);
    if (rule) {
      const meta = CATEGORY_META[rule.category];
      return {
        category: rule.category,
  ...
  if (reasonCode) {
    const rule = matchRule(RETURN_CODE_RULES, reasonCode, reasonMessage || returnMessage || providerMessage);
    if (rule) {
      const meta = CATEGORY_META[rule.category];
      return {
        category: rule.category,
...
// ==========================================
// UX Action Types for Frontend
// ==========================================

export type UxAction =
  | 'retry_same_card' // Permitir retentativa com mesmo cartão
  | 'retry_later' // Tentar novamente mais tarde (tempo definido)
  | 'change_card' // Sugerir trocar cartão
  | 'change_method' // Sugerir outro método de pagamento
  | 'no_retry' // Não permitir retentativa (irreversível)
  | 'contact_bank'; // Contatar banco

export type UxActionConfig = {
  action: UxAction;
  allowRetrySameCard: boolean;
  allowRetryLater: boolean;
  suggestChangeCard: boolean;
  suggestChangeMethod: boolean;
  retryAfterMs: number | null; // null = imediato, 0 = não permitir, >0 = tempo em ms
  buttonLabel: string;
  secondaryButtonLabel: string | null;
};

// Mapeamento de RetryPolicy → UxActionConfig
const UX_ACTION_MAP: Record<RetryPolicy, UxActionConfig> = {
  no_retry: {
    action: 'no_retry',
    allowRetrySameCard: false,
    allowRetryLater: false,
    suggestChangeCard: true,
    suggestChangeMethod: true,
    retryAfterMs: 0,
    buttonLabel: 'Tentar outro cartão',
    secondaryButtonLabel: 'Escolher outro método',
  },
  no_auto_retry: {
    action: 'change_card',
    allowRetrySameCard: false,
  ...
  user_retry_allowed: {
    ...
    allowRetrySameCard: true,
    allowRetryLater: true,
    suggestChangeCard: false,
    suggestChangeMethod: false,
    retryAfterMs: null,
    buttonLabel: 'Tentar novamente',
    secondaryButtonLabel: 'Usar outro cartão',
  },
  user_retry_later: {
    action: 'retry_later',
    allowRetrySameCard: false,
    ...
    suggestChangeCard: false,
    suggestChangeMethod: false,
    retryAfterMs: 60000, // 1 minuto
    buttonLabel: 'Tentar novamente',
    secondaryButtonLabel: 'Tentar mais tarde',
  },
  retry_with_limit_4x_16d: {
    action: 'retry_same_card',
    allowRetrySameCard: true,
  ...
  retry_next_day_4x_16d: {
    ...
    suggestChangeCard: true,
    suggestChangeMethod: true,
    retryAfterMs: 86400000, // 24 horas
    buttonLabel: 'Tentar amanhã',
    secondaryButtonLabel: 'Usar outro cartão',
  },
  retry_after_few_seconds: {
    action: 'retry_later',
  ...
  retry_after_1h: {
    ...
    suggestChangeCard: true,
    suggestChangeMethod: true,
    retryAfterMs: 3600000, // 1 hora
    buttonLabel: 'Tentar em 1 hora',
    secondaryButtonLabel: 'Usar outro cartão',
  },
  retry_after_5m: {
    action: 'retry_later',
  ...
  retry_after_72h: {
    ...
    suggestChangeCard: true,
    suggestChangeMethod: true,
    retryAfterMs: 259200000, // 72 horas
    buttonLabel: 'Tentar em 3 dias',
    secondaryButtonLabel: 'Usar outro método',
  },
  retry_after_30d: {
    action: 'retry_later',
    allowRetrySameCard: false,
    ...
    suggestChangeCard: true,
    suggestChangeMethod: true,
    retryAfterMs: 2592000000, // 30 dias
    buttonLabel: 'Contatar banco',
    secondaryButtonLabel: 'Usar outro método',
  },
};

/**
 * Converte RetryPolicy em ações de UX para o frontend
 * @param policy Política de retry retornada pela classificação
 * @param currentRetryCount Número atual de tentativas (para limites)
 * @param maxRetries Máximo de tentativas permitidas (default: 15)
 */
export function getUxActionFromPolicy(
  policy: RetryPolicy,
  currentRetryCount: number = 0,
  maxRetries: number = 15
): UxActionConfig {
  const baseConfig = UX_ACTION_MAP[policy] || UX_ACTION_MAP.no_auto_retry;

  // Se excedeu limite de tentativas, forçar troca de cartão
  if (currentRetryCount >= maxRetries) {
    return {
      ...
      suggestChangeMethod: true,
      retryAfterMs: 0,
      buttonLabel: 'Limite excedido. Use outro cartão',
      secondaryButtonLabel: 'Escolher outro método',
    };
  }
...
/**
 * Combina classificação de decline com ação de UX
 * Retorna objeto completo para o frontend renderizar o modal de erro
 */
export type DeclineWithUxAction = DeclineClassification & {
  uxAction: UxActionConfig;
  retryCount: number;
  canRetry: boolean;
};

export function classifyDeclineWithUx(
  input: Parameters<typeof classifyDecline>[0],
  currentRetryCount: number = 0,
  maxRetries: number = 15
): DeclineWithUxAction {
  const classification = classifyDecline(input);
  const uxAction = getUxActionFromPolicy(
    classification.retryPolicy,
    currentRetryCount,
    maxRetries
  );
  ...
  return {
    ...
    canRetry: uxAction.allowRetrySameCard || uxAction.allowRetryLater,
  };
}

// ==========================================
// 3DS Error Classification
// ==========================================
...
const THREE_DS_ERROR_MAP: Record<ThreeDsErrorType, {
  category: DeclineCategory;
  message: string;
  retryPolicy: RetryPolicy;
}> = {
  authentication_failed: {
    category: 'three_ds_failed',
    message: 'A verificacao de seguranca falhou. Verifique seus dados e tente novamente.',
    retryPolicy: 'user_retry_allowed',
  },
  timeout: {
    category: 'three_ds_failed',
    message: 'A verificacao de seguranca demorou muito. Tente novamente.',
    retryPolicy: 'retry_after_few_seconds',
  },
  not_enrolled: {
    category: 'processing_error',
    message: 'Seu cartao nao esta habilitado para compras online. Contate seu banco.',
    retryPolicy: 'no_auto_retry',
  },
  technical_error: {
    category: 'processing_error',
    message: 'Erro no sistema de verificacao. Tente novamente mais tarde.',
    retryPolicy: 'user_retry_later',
  },
  canceled_by_user: {
    category: 'auth_failed',
    message: 'A verificacao foi cancelada. Tente novamente.',
    retryPolicy: 'user_retry_allowed',
  },
  invalid_card_range: {
    category: 'invalid_card_data',
    message: 'Cartao nao suportado para esta transacao. Use outro cartao.',
    retryPolicy: 'no_auto_retry',
  },
  unsupported_brand: {
    category: 'invalid_card_data',
    message: 'Bandeira nao suportada para verificacao de seguranca. Use outro cartao.',
    retryPolicy: 'no_auto_retry',
  },
};

/**
 * Classifica erros específicos de autenticação 3DS
 * @param errorType Tipo do erro 3DS retornado pelo SDK bpmpi
 * @param rawMessage Mensagem original do erro (opcional)
 */
export function classify3DsError(
  errorType: ThreeDsErrorType | string,
  rawMessage?: string
): DeclineClassification {
  const normalizedType = errorType.toLowerCase().replace(/[-_\s]/g, '_') as ThreeDsErrorType;
  const errorConfig = THREE_DS_ERROR_MAP[normalizedType] || THREE_DS_ERROR_MAP.technical_error;
  const meta = CATEGORY_META[errorConfig.category];

  return {
    category: errorConfig.category,
    uxKey: meta.uxKey,
    retryPolicy: errorConfig.retryPolicy,
    message: rawMessage || errorConfig.message,
    matchedCode: normalizedType,
  };
}

/**
 * Verifica se um código/erro é relacionado a 3DS
 */
export function is3DsError(code: string | null): boolean {
